...
<div className="review flex flex-col md:flex-row justify-between items-start mt-8 space-y-6 md:space-y-0 md:space-x-6">
  {/* Ratings Section */}
  <div className="reviews p-8 bg-white rounded-lg shadow-lg w-full md:w-1/3 h-[450px] max-[425px]:h-[290px]">
    <div className="flex items-center mb-4">
      {[...Array(4)].map((_, index) => (
        <svg key={index} className="w-5 h-5 text-yellow-400 mr-1" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
          <path d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z" />
        </svg>
      ))}
      <svg className="w-5 h-5 text-gray-300 mr-1" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
        <path d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z" />
      </svg>
      <p className="ml-1 text-sm font-semibold text-gray-800">4.95 out of 5</p>
    </div>
    <p className="text-sm text-gray-600">1,745 global ratings</p>
    {[
      ...
      <div key={index} className="flex items-center mt-4">
        ...
        <div className="w-2/4 h-5 mx-4 bg-gray-200 rounded">
          <div className="h-5 bg-yellow-400 rounded" style={{ width: rating.width }}></div>
        </div>
  ...
  <div className="p-8 bg-white rounded-lg shadow-lg w-full md:w-2/3">
    <h3 className="text-lg font-semibold text-gray-800 mb-4">Leave Your Feedback</h3>
    <form id="feedbackForm" className="space-y-6" action="https://bookmycater.freewebhostmost.com/reviewsubmit.php" method="POST">
      <div>
        <label className="block text-sm font-medium text-gray-700" htmlFor="name">Name</label>
        <input
          type="text"
          id="name"
          className="mt-2 p-3 w-full border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
        ...
        <div className="flex space-x-3 mt-2">
          {[1, 2, 3, 4, 5].map((value) => (
            <label key={value} className="flex items-center">
              <input type="radio" name="rating" value={value} required className="mr-1" /> {value}
            </label>
          ))}
        </div>
      </div>
...
<div className="comments w-full mt-8 p-4 m-3">
  {reviews.map(review => (
    <div key={review.id} className="comment bg-white p-6 rounded-lg shadow-md mb-6">
      <div className="user_info flex flex-row items-center space-x-4">
        ...
        <div className="names flex flex-col">
          <p className="name text-lg font-semibold text-gray-800">{review.name}</p>
          <p className="date text-sm text-gray-500">{new Date(review.date).toLocaleDateString()}</p>
        </div>
        <div className="user_ratings flex items-center ml-auto text-yellow-500">
          <span>{'⭐ '.repeat(review.rating)}</span>
          <p className="text-sm ml-1 text-gray-700">{review.rating}.0</p>
        </div>
      </div>
      <p className="comment_text mt-4 text-gray-700">{review.feedback}</p>
    </div>
  ))}
</div>
